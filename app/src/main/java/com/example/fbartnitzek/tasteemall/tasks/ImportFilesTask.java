...
public class ImportFilesTask extends AsyncTask<List<File>, Void, String> {
    ...
    protected String doInBackground(List<File>... params) {
        ...
        File producerFile = getEntryFile(files, ExportToDirTask.EXPORT_PREFIX + producers);
        File drinkFile = getEntryFile(files, ExportToDirTask.EXPORT_PREFIX + drinks);
        File reviewFile = getEntryFile(files, ExportToDirTask.EXPORT_PREFIX + reviews);
        
        // can be done separate...
        String message = importEntries(DatabaseContract.ProducerEntry.CONTENT_URI,
                    QueryColumns.ExportAndImport.ProducerColumns.COLUMNS,
                    producerFile, producers);
